...
| FEMU and SVD | incomplete | blocked | open | — | not applicable: no boundary-only driver | `reference/numerics/femu_sensitivity_and_svd` | `explanation/identification/femu_identification`, `explanation/identification/dic_weighted_tensor_observability`, `explanation/identification/srix_parametric_observability` | `reference/evidence/femu_qualification`, `reference/evidence/dic_observable_modes`, `reference/evidence/srix_parametric_observability` | production boundary-only driver remains blocked; periodic and wrap-free spectral DIC surrogates are available offline, but held-out modal noise does not calibrate absolute detectability, and image-level qualification, the FEMU workflow and experimental identification remain open |
