...
| FEMU and SVD | incomplete | blocked | open | — | not applicable: no boundary-only driver | `reference/numerics/femu_sensitivity_and_svd` | `explanation/identification/femu_identification`, `explanation/identification/dic_observable_femu_modes` | `reference/evidence/femu_qualification`, `reference/evidence/dic_observable_modes` | production boundary-only driver remains blocked; DIC-weighted observability is supported for archived post-processing |
