...
| FEMU and SVD | incomplete | blocked | open | — | not applicable: no boundary-only driver | `reference/numerics/femu_sensitivity_and_svd` | `explanation/identification/femu_identification`, `explanation/identification/dic_weighted_tensor_observability`, `explanation/identification/srix_parametric_observability` | `reference/evidence/femu_qualification`, `reference/evidence/dic_observable_modes`, `reference/evidence/srix_parametric_observability` | production boundary-only driver remains blocked; full DIC-weighted parametric SVD is also blocked by the unhydrated noise payload |
