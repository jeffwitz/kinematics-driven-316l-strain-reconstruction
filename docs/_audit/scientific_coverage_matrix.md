...
| FEMU and SVD | incomplete | blocked | open | — | not applicable: no boundary-only driver | `reference/numerics/femu_sensitivity_and_svd` | `explanation/identification/femu_identification`, `explanation/identification/dic_weighted_tensor_observability` | `reference/evidence/femu_qualification`, `reference/evidence/dic_observable_modes` | production boundary-only driver remains blocked; free-field DIC observability is a prerequisite, not parametric FEMU observability |
