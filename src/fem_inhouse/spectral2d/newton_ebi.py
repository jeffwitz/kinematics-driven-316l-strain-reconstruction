...
class EBISpectralSolverConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not 0.0 < self.forcing_minimum <= self.forcing_maximum < 1.0:
            raise ValueError("forcing bounds must satisfy 0 < minimum <= maximum < 1")
        if not 0.0 < self.forcing_initial <= self.forcing_maximum:
            raise ValueError("forcing initial value must be within forcing bounds")
        if not 0.0 < self.forcing_gamma <= 1.0:
