...
from fem_pixel import _vm, run_fem
...
if os.environ.get("FEM_REUSE_FRAMES", "0") == "1":
    ...
    if result is not None:
        print(
            f"[driver] reusing final-state fields saved by visual_fem_test "
...
if all(v is not None for v in ab.values()):
    ...
    for k in ("u1", "u2", "s11", "s22", "s12", "e11", "e22", "e12"):
        ...
        if a.shape != f.shape:
            print(
                f"{k:>6}  SHAPE MISMATCH ab{a.shape} fem{f.shape} "
    ...
    if ab["s11"].shape == fem["s11"].shape:
        svm_ab = np.sqrt(
            ab["s11"] ** 2 + ab["s22"] ** 2 - ab["s11"] * ab["s22"] + 3 * ab["s12"] ** 2
        )
