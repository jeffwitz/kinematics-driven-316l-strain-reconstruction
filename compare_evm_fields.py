...
from test_config import (NX as FEA_NX, NY as FEA_NY, JOB_NAME, FEM_TAG,
                         DIC_DIR as dic_dir, DIC_FINAL_FRAME as dic_final_frame,
                         PX_TO_MM, crop_center, window_tag,
                         AB_OUT, FEM_OUT, FINAL_VALIDATION_DIR)
...
if n_base:
    exx_d = exx_d - base[0]; exy_d = exy_d - base[1]; eyy_d = eyy_d - base[2]
...
if DIC_EVM_SMOOTH_SIGMA > 0:
    ...
    evm_dic = np.where(w > 1e-6,
                       gaussian_filter(filled, DIC_EVM_SMOOTH_SIGMA) / np.maximum(w, 1e-6),
                       np.nan)
...
for t, f in fields:
    if t != "DIC" and f.shape != (FEA_NX, FEA_NY):
        print(f"[warn] {t} field shape {f.shape} != current window "
...
fig, axes = plt.subplots(1, len(fields), figsize=(11 * len(fields), 10.5),
                         constrained_layout=True)
if len(fields) == 1:
    axes = [axes]

for ax, (title, f) in zip(axes, fields):
    im = ax.imshow(100.0 * f.T, cmap="jet", origin="lower",
    ...
    ax.text(0.02, 0.98, f"mean = {mean_pct:.4f} %", transform=ax.transAxes,
...
for ext in ("pdf", "png"):
    fig.savefig(os.path.join(OUT_DIR, f"evm_fields_dic_abaqus_fem_{window_tag()}.{ext}"),
                dpi=250)
plt.close(fig)
...
if all(a is not None for a in (ab_e11, ab_e22, ab_e12)):
    print(f"  {'Abaqus':7s} {np.nanmean(ab_e11):12.3e} {np.nanmean(ab_e22):12.3e} {np.nanmean(ab_e12)/2:12.3e}")
if all(a is not None for a in (fm_e11, fm_e22, fm_e12)):
    print(f"  {'FEM':7s} {np.nanmean(fm_e11):12.3e} {np.nanmean(fm_e22):12.3e} {np.nanmean(fm_e12)/2:12.3e}")
...
u2_dic = crop_center(_pad_to(np.load(os.path.join(dic_dir, f"U_{dic_final_frame}.npy")).astype(float),
                              FEA_NX + 1, FEA_NY + 1),
                     FEA_NX + 1, FEA_NY + 1) * PX_TO_MM
...
if os.path.isfile(p):
    ...
    if a.shape == (FEA_NX + 1, FEA_NY + 1):
        ...
        print(f"[warn] Abaqus u2 shape {a.shape} != window ({FEA_NX+1},{FEA_NY+1}) - skipping panel.")
...
u2_fields = [(t, f) for t, f in (("DIC", u2_dic), ("Abaqus", u2_ab), ("FEM", u2_fem))
             if f is not None]

def _u2_report(name, a, b):
    d = np.abs(a - b)
    inner = d[1:-1, 1:-1]
    bnd = d.copy(); bnd[1:-1, 1:-1] = np.nan
    print(f"\n{name}:")
    print(f"  overall : max|diff| = {np.nanmax(d):.4e} mm   rms = {np.sqrt(np.nanmean(d**2)):.4e} mm")
    print(f"  boundary: max|diff| = {np.nanmax(bnd):.4e} mm  (should be ~0: BCs are prescribed)")
    print(f"  interior: max|diff| = {np.nanmax(inner):.4e} mm   rms = {np.sqrt(np.nanmean(inner**2)):.4e} mm")
...
if _sim is not None:
    _u2_report(f"u2 {'FEM' if u2_fem is not None else 'Abaqus'} vs DIC "
               f"(prediction vs measurement)", _sim, u2_dic)
...
if len(u2_fields) > 0:
    ...
    fig, axes = plt.subplots(1, n_p, figsize=(11 * n_p, 10.5),
                             constrained_layout=True)
    axes = np.atleast_1d(axes)
    for ax, (title, f) in zip(axes, u2_fields):
        im = ax.imshow(f.T, cmap="jet", origin="lower",
                       vmin=vmin2, vmax=vmax2, interpolation="nearest")
        ax.set_title(f"{title}  u2", fontsize=22)
        ax.set_xlabel("x (node)", fontsize=16); ax.set_ylabel("y (node)", fontsize=16)
        ax.tick_params(labelsize=13)
        ax.text(0.02, 0.98, f"mean = {np.nanmean(f):.4e} mm", transform=ax.transAxes,
                fontsize=15, va="top", ha="left",
                bbox=dict(facecolor="white", alpha=0.7, edgecolor="none"))
    ...
    if _sim is not None:
        ...
        im2 = ax.imshow(np.abs(_sim - u2_dic).T, cmap="magma", origin="lower",
    ...
    for ext in ("pdf", "png"):
        fig.savefig(os.path.join(OUT_DIR, f"u2_fields_dic_abaqus_fem_{window_tag()}.{ext}"),
                    dpi=250)
    plt.close(fig)
